...
const projectSchema = z.object({
  ...
  image: z
    .instanceof(File, { message: "Main project image is required" })
    .nullable()
    .optional(),
  ...
  const onSubmit = async (data: ProjectFormData) => {
    try {
      ...
      console.log("Form Data:", {
        ...
        mainImage: data.image?.name,
        galleryImages: data.images?.map((img) => img.name),
        technologies: data.technologies,
        details: data.details,
      });

      // add project the database
      ...
      <Card className="bg-black/40 border-white/10 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription className="text-white/70">
            Fill in the details for your new project. All fields marked with *
            are required.
          </CardDescription>
        </CardHeader>
        <CardContent>
